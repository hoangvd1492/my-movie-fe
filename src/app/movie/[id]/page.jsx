    ...
    return (
        <div className="relative h-screen w-full  text-primary " >
            <div
                ...
                className="absolute inset-0 bg-cover bg-center mask-b-from-20% mask-b-to-90% z-9"
            ></div>
            <div className="relative z-10 pt-[15%] px-8 max-lg:px-2">
            ...
            <div id="player" className="w-full px-8 max-lg:px-2 mb-16 mt-64">
                <Player embedUrl={`https://multiembed.mov/?video_id=${encodeURIComponent(id)}&tmdb=1`} />
            </div>

            <div className="px-8 max-lg:px-2 text-primary flex flex-col gap-4">
                <div className="text-xl font-[700]">Diễn viên/Lồng tiếng:</div>
                <div className="flex flex-row gap-4 flex-wrap">
                    {
                        cast.map((actor, i) => {
                            return (
                                <div key={i} className="h-64 w-32 flex flex-col items-center gap-1">
                                    <img className="rounded-[4px] h-48" src={`https://image.tmdb.org/t/p/original${actor.profile_path}`} alt={actor.original_name} />
